...
const adviceE2E = new Trend("advice_e2e_ms");  // ✅ 추가
...
export const options = {
  scenarios: {
    safe: {
      ...
      vus: 3,               // VU 1~3
      iterations: 5,        // iteration 3~5
      maxDuration: "10m",
    },
  },
  thresholds: {
    http_req_failed: ["rate<0.01"],
    advice_e2e_ms: ["p(95)<90000"],   // ✅ 이제 유효
  },
};
...
function postAdvice(payload) {
  ...
  check(res, { "POST 201/200": (r) => r.status === 201 || r.status === 200 });
  return res.json();
}
...
export default function () {
  ...
  const elapsed = Date.now() - start; // ✅ E2E 끝
  adviceE2E.add(elapsed);             // ✅ 메트릭 기록
